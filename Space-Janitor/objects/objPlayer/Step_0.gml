...
if (!onGround) {
    if ((cLeft || cRight) && vy >= 0) {
        // Wall slide
        vy = scrApproach(vy, vyMax, gravSlide);
    } else {
        // Fall normally
...
if (kLeft && !kRight && !sticking) {
    ...
    if (vx > 0)
        vx = scrApproach(vx, 0, tempFric);   
    vx = scrApproach(vx, -vxMax, tempAccel);
// Right
...
if (kJump) { 
    if (onGround) {
        // Fall thru platform
        if (kDown) {
            if (place_meeting(x, y + 1, objJumpThrough))
                ++y;
        } else {
            vy = -jumpHeight;
...
// Run particles
    ...
    instance_create_layer(x, y + 8, "mainInstances", objParticlePlayer);
...
xscale = scrApproach(xscale, 1, 0.05);
yscale = scrApproach(yscale, 1, 0.05);

/* */
/// Hitbox

with (objPlayerAtkBox)
    instance_destroy();

// Dash out of roll
//if (sprite_index == sPlayerRollSlash) {    
//    with (instance_create_layer(x, y, "mainInstances", objPlayerAtkBox)) {
